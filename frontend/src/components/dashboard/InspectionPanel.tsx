    ...
    !plant ? (
      ...
      <div className="grid gap-3">
        ...
        <button
          type="button"
          onClick={onInspect}
          disabled={!canInspect || !plant.imageUrl}
          className="inline-flex items-center justify-center rounded-lg border border-[#BFD6BA] bg-[#EAF5EA] px-3 py-2 text-xs font-semibold text-[#2E7D32] hover:bg-[#DDEEDD] disabled:cursor-not-allowed disabled:opacity-60"
        >
          {plant.inspected ? "Re-run Mock CV" : "Run Mock CV Inspection"}
        </button>
      </div>
    )
  );
